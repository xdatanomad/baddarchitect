...
| `/post done <permalink>` | Record a manual publish (video, API outage, etc.). Validate the URL looks like a LinkedIn post (`https://www.linkedin.com/…`). Route to `li:published` the same way the sweep does — **there is no direct edge, it must pass through `li:publishing`**: `transition <n> li:publishing --from <state>` (state is `approved`/`scheduled`/`blocked`), then `li-issue.sh copy-get <n> > <copy>`, `li-worklog.sh publish --issue <n> --url <permalink> --copy-file <copy> --commit`, then `transition <n> li:published --from li:publishing` (closes the issue). Post a confirming comment with the permalink. |
